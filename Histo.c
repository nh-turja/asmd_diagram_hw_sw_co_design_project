...
// Allocate arrays
   ...
   if ( (PNDiff_arr_out = (short *)calloc(sizeof(short), MAX_PNDIFF_VALS)) == NULL )
      { printf("ERROR: Failed to calloc data 'PNDiff_arr_out' array!\n"); exit(EXIT_FAILURE); }
...
// Wait for 'stopped' to be asserted by hardware. When this occurs, computePNDiff FSM is finished and its ready to transfer
// data out.
   while ( ((*DataRegA) & (1 << IN_SM_HANDSHAKE)) == 0 );
...
// After computing the histogram, Controller expects to transfer histogram memory and distribution parameters back to C program
   load_unload = 1;
   LoadUnloadBRAM(MAX_STRING_LEN, MAX_PNDIFF_VALS, num_vals, load_unload, PNDiff_arr_out, CtrlRegA, DataRegA, ctrl_mask);

// Read the data from the PNDIFF file
   num_vals = ReadData(MAX_STRING_LEN, MAX_DATA_VALS, infile_name, PNDiff_arr_out);
// ==================================================================================
// Software computed values. Hardware reports mean WITH 4 bits of precision but range using ONLY the integer portion.
   gettimeofday(&t0, 0);
   ComputeHisto(MAX_DATA_VALS, num_vals, PNDiff_arr_out, (short)LV_BOUND, (short)HV_BOUND, (short)DIST_RANGE, precision_scaler,
      software_histo);
   gettimeofday(&t1, 0); elapsed = (t1.tv_sec-t0.tv_sec)*1000000 + t1.tv_usec-t0.tv_usec; 
   printf("\tSoftware Runtime %ld us\n\n", (long)elapsed);
// ==================================================================================
//Print out the PNDiff.
int i;
printf("PNDIFF VALUES: \n");
for(i=0;i<MAX_PNDIFF_VALS;i++)
{
	printf("(%4d) %3d  ", i, PNDiff_arr_out[i]);
      if ( ((i+1) % 10) == 0 )
         printf("\n");
}
